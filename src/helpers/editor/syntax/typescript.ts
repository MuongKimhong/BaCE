...
function handleIdentifier(
  ...
  if (nextChar === "(") {
    ...
    if (TYPESCRIPT_KEYWORDS.includes(value)) {
...
export function tokenizeTypeScriptCode(input: string): Token[] | null {
  const tokens: Token[] = [];
  ...
  if (!syntaxHighlight || !langTokenTypes) {
    ...
    langTokenTypes = syntaxHighlight.languages.find((lang) => lang.language === "typescript");
